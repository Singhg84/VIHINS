...
$sqlPrint = "SELECT * FROM steps_student_master WHERE stud_sub_id = '".$subject."' and stud_session_admission = '".$admissionsession."' and stud_status = 'Active' and stud_id = ".$studentId;
					...
					<tr>
						<td colspan="9">&nbsp;</td>
							...
							<tr style="font-size:14px;">
								<td colspan="9" >The following is the statement of marks and grades obtained by: <b>'.strtoupper($rowPrint['stud_name']).'</b> <br/>Roll:<b>'.$rollp.' NO. '.$rowPrint['stud_roll_no'].' </b>Registration No.:<b>'.str_pad($rowPrint['stud_reg_no'], 7, '0', STR_PAD_LEFT).'</b> of <b>'.$rowPrint['stud_reg_year'].'</b> in  <b>M.Sc. '.$semp.'</b> SEMESTER EXAMINATION, '.$yrf.'.<BR></td>
							</tr>
							</table>
			...
			while($rowPrintcoursedata = $resultPrintcoursedata->fetch_assoc()){
				if($coursethcount == 1 && $rowPrintcoursedata['course_paper_type'] == 'Theory'){
					...
					$output .='<tr>
						...
						<td rowspan="2" style="vertical-align:middle;text-align:center;"><br/><br/>'.($resultPrintcourseunitdata['c1']==0?'':$resultPrintcourseunitdata['c1']).'<br/><br/>'.(strlen($resultPrintcourseunitdata['c5'])>30?'<br/>':'').($resultPrintcourseunitdata['c2']==0?'':$resultPrintcourseunitdata['c2']).'<br/></td>';
						if($rowPrintcoursedata['course_unit'] == 2){
