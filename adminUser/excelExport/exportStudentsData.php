...
if (!$result = mysqli_query($conn, $query)) {
	...
	header("location:../marksEntry/exportCSVStudents.php");
	exit;
}
...
if($first_course_unit_id == '0'){
header('Content-Disposition: attachment; filename = '.$rowcourse['course_no'].'_'.$first_course_unit_id.'_'.time().'.csv');
}
else{
header('Content-Disposition: attachment; filename = '.$rowcourse['course_no'].'_'.$rowunit['course_unit_id'].'_'.time().'.csv');
}
...
if($rowcourse['course_unit']== 0){
fputcsv($output, array('Students ID.','Enrollment No.','Students Name','Roll No.','Reg. No.','Reg. Year','IA(F.M-'.$rowcourse['course_IA'].')','ESE(F.M-'.$rowcourse['course_ESE'].')'));
}

else if($rowcourse['course_paper_type']=='Practical'){
