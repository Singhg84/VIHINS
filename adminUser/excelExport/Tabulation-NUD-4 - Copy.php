					...
					for($it=0;$it<$maxsubject;$it++){
						...
						if((isset($theoryId[$it]) && $theoryId[$it] != '') && (isset($practicalId[$it]) && $practicalId[$it] != '')){
							...
							if($it == 2){
								...
								$output	.= 	'<td rowspan="2">'.$stmtstudentTotalmarks2['rt_result'].'</td>
								...
								$output	.= 	'<td rowspan="4">'.number_format((float)round(($stmtstudentTotalmarks1['rt_sgpa']+$stmtstudentTotalmarks2['rt_sgpa']+$stmtstudentTotalmarks3['rt_sgpa']+$stmtstudentTotalmarks4['rt_sgpa'])/4,2, PHP_ROUND_HALF_DOWN),2,'.',',').'</td>';
								$output	.= 	'</tr>
										<tr>
											<td>'.$resultPrintmarksdata['md2'].'</td>
											<td>'.$resultPrintmarksdata['md4'].'</td>
...
if (@file_exists(dirname(__FILE__).'/lang/eng.php')) {
    ...
    $pdf->setLanguageArray($l);
		...
		ob_end_clean();
		$pdf->Output('Tabulation-4th-semester.pdf', 'I'); 
