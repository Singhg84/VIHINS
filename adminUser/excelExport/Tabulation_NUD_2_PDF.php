...
include("../../connect.php"); 
//require("../../htmltopdf1/vendor/autoload.php");
 $output = '';  
$yr = explode("-",$_POST['semester_session']);	
//$yr1 = substr($yr[1],2);
if($_POST['semester_list'] == 2){
	$semp = '2<sup>nd</sup>';
...
<html lang="en">
    <head>
        <title>NUD 2nd Semester :: Tabulation</title>
		...
		$sqlPrint = "SELECT stud_id,stud_roll_no,stud_name,stud_reg_no,stud_reg_year,stud_category,SUM(rm_total_marks) rt,SUM(rm_credit_points) rc FROM 
			...
			$output .= '<div class="printpage" id="page-1">
						<CENTER>
							...
							<div class="text2"><b>M.SC. '.$semp.' SEMESTER EXAMINATION '.$yrf.' IN NUTRITION AND DIETETICS</b>
							</div>
							<br>
						</CENTER>
						<table cellspacing="0">
          ...
          $output .= '<tr>
                    ...
                    <tr>
                        <td colspan="4">'.$rollp.' NO. '.str_pad($rowPrint['stud_roll_no'], 4, '0', STR_PAD_LEFT).'</td>
					...
					for($it=0;$it<$maxsubject;$it++){
						...
						if(isset($theoryId[$it]) && $theoryId[$it] != ''){
							if($it == 0 && ($theoryId[$it]['course_unit'] == 2) && !empty($practicalId[$it]) && ($practicalId[$it]['course_unit'] == 2)){
							...
							$output	.=   '<tr>
											...
											<td rowspan="2">'.$resultPrintmarksPracdatamaster['rm_credit_points'].'</td>
											<th rowspan="8">'.$stmtstudentTotalmarks2['rt_marks_obtain'].'</th>
											<th rowspan="8">'.$totalcredit.'</th>
											<th rowspan="8">'.$stmtstudentTotalcreditPoint['CP'].'</th>
											<th rowspan="8">'.$stmtstudentTotalmarks2['rt_sgpa'].'</th>
											<th rowspan="4">'.$stmtstudentTotalmarks2['rt_result'].'</th>
							...
							if($it == 1 && ($theoryId[$it]['course_unit'] == 2) && !empty($practicalId[$it]) && ($practicalId[$it]['course_unit'] == 2)){
								$output	.= '<tr>
											<th rowspan="2">'.$theoryId[$it]['course_no'].'</th>
											<td>'.$resultPrintmarksdata['md1'].'</td>
											<td>'.$resultPrintmarksdata['md3'].'</td>
											...
											<td rowspan="2">'.$practicalId[$it]['course_credit'].'</td>
											<td rowspan="2">'.$resultPrintmarksPracdatamaster['rm_credit_points'].'</td>
											<th rowspan="2">'.$stmtstudentTotalmarks1['rt_sgpa'].'</th>
											<th rowspan="2">'.$stmtstudentTotalmarks2['rt_sgpa'].'</th>
											</tr>
										<tr>
											<td >'.$resultPrintmarksdata['md2'].'</td>
											<td >'.$resultPrintmarksdata['md4'].'</td>
							...
							if(($theoryId[$it]['course_unit'] == 2) && empty($practicalId[$it])){
						$output	.=		 '<tr>
											<th rowspan="2">'.$theoryId[$it]['course_no'].'</th>
											<td>'.$resultPrintmarksdata['md1'].'</td>
											<td>'.$resultPrintmarksdata['md3'].'</td>
											...
											<th rowspan="4" colspan="7">&nbsp;</th>
											<th rowspan="4">'.$stmtstudentTotalmarks2['rt_percent'].'</th>
											<th rowspan="2">'.$stmtstudentTotalmarks1['rt_result'].'</th>
											<th rowspan="2">'.$stmtstudentTotalmarks2['rt_result'].'</th>
										</tr>
										<tr>
											<td >'.$resultPrintmarksdata['md2'].'</td>
											<td >'.$resultPrintmarksdata['md4'].'</td>
							...
							if(($theoryId[$it]['course_unit'] != 2) && empty($practicalId[$it])){
							$output	.= 	'<tr>
											<th rowspan="2" style="padding-top:10px; padding-bottom:10px;">'.$theoryId[$it]['course_no'].'</th>
											<td rowspan="2">'.$resultPrintmarksdata['md1'].'</td>
											<td rowspan="2">'.$resultPrintmarksdata['md3'].'</td>
											<td rowspan="2">'.$resultPrintmarksdatamaster['rm_total_marks'].'</td>
											<td rowspan="2">'.$resultPrintmarksdatamaster['rm_letter_grade'].'</td>
											<td rowspan="2">'.$resultPrintmarksdatamaster['rm_grade_points'].'</td>
											<td rowspan="2">'.$theoryId[$it]['course_credit'].'</td>
											<td rowspan="2">'.$resultPrintmarksdatamaster['rm_credit_points'].'</td>
											<th rowspan="2">'.$stmtstudentTotalmarks1['rt_percent'].'</th>
											<th rowspan="2">'.$stmtstudentTotalmarks2['rt_percent'].'</th>
										</tr>
										<tr>
        								</tr>';
					...
					$totmarks = $stmtstudentTotalmarks2['rt_full_marks']+$stmtstudentTotalmarks1['rt_full_marks'];
					$totobtmarks = $stmtstudentTotalmarks2['rt_marks_obtain']+$stmtstudentTotalmarks1['rt_marks_obtain'];
